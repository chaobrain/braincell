...
nodes — together with the ion-construction and channel-binding helpers it
drives.

The layout vocabulary this module builds on (``MechanismLayout``,
``ClampRoutingTable``, and the buffer allocation / clamp evaluation helpers)
lives in :mod:`braincell._compute.layouts` and is re-exported below so existing
importers of ``braincell._compute.runtime`` keep working.
"""
