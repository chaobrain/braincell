...
"""Runtime state and mechanism lowering for multi-compartment cells.

Holds :class:`CellRuntimeState` — the bridge object that turns a ``Cell``'s
declarations into lowered layouts, state buffers, and instantiated runtime
nodes — together with the channel-binding helpers it drives.

Three vocabularies this module builds on now live in sibling leaf modules and
are re-exported below so existing importers of ``braincell._compute.runtime``
keep working:

- ``MechanismLayout``, ``ClampRoutingTable``, and the buffer allocation / clamp
  evaluation helpers live in :mod:`braincell._compute.layouts`.
- Runtime ion instantiation, ion parameter normalization, broadcast/scatter, and
  per-layout ion synchronization live in :mod:`braincell._compute.ions`.
- Channel/ion binding resolution, runtime node instantiation, merged
  channel-layout installation, and per-node parameter synchronization live in
...
from .bindings import (  # noqa: F401
    _BoundIonChannelCurrentComponentRuntime,
    _BoundIonChannelRuntime,
    _CONDUCTANCE_PARAM_NAMES,
    _build_runtime_nodes,
    _channel_current_owner_family,
    _channel_current_owner_specs,
    _channel_family_slots,
    _configure_runtime_subsolvers,
    _initial_merged_channel_param,
    _install_merged_channel_nodes,
    _instantiate_runtime_node,
    _is_mergeable_channel_layout,
    _is_root_level_runtime_node,
    _merged_channel_constructor_params,
    _merged_channel_param_value,
    _owner_channel_executable,
    _partition_non_overlapping_channel_layouts,
    ...
    _scatter_active_channel_param,
    _sync_runtime_node_param,
    _unique_ion_channel_key,
)
from .bridge import (
...
from .ions import (  # noqa: F401
    ...
    _restore_shaped_species_initializers,
    _runtime_ion_family,
...
## scatter/gather/quantity/geometry helpers moved to braincell._compute.bridge
## layout records, clamp routing/evaluation, and state-buffer allocation
##   moved to braincell._compute.layouts
