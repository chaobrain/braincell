...
"""Package-level guards for :mod:`braincell.filter`.
...
``from braincell.filter import *``. Neither shows up in a normal test run,
which is why this guard exists.
"""

import unittest

import braincell.filter
from braincell._testing import ReExportTests


class FilterReExportTest(ReExportTests, unittest.TestCase):
    """``braincell.filter.__all__`` resolves, has no duplicates, and stays sorted."""

    package = braincell.filter
