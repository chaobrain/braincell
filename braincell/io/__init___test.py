...
"""Package-level guards for :mod:`braincell.io`.
...
``from braincell.io import *``. Neither shows up in a normal test run,
...
class IoReExportTest(ReExportTests, unittest.TestCase):
    """``braincell.io.__all__`` resolves, has no duplicates, and stays sorted."""

    package = braincell.io
