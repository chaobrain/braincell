...
"""Package-level guards for :mod:`braincell.mech`.
...
``from braincell.mech import *``. Neither shows up in a normal test run,
which is why this guard exists.
"""

import unittest

import braincell.mech
from braincell._testing import ReExportTests


class MechReExportTest(ReExportTests, unittest.TestCase):
    """``braincell.mech.__all__`` resolves, has no duplicates."""

    package = braincell.mech
