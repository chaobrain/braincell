...
"""Sweep helper for channel validation between NEURON and braincell.
...
def resolve_braincell_channel_class(channel_name: str):
    ...
    if hasattr(braincell.channel, resolved):
        return getattr(braincell.channel, resolved)
...
def convert_braincell_channel_params(
    channel_cls,
    params: Dict[str, Any],
    *,
    temperature_celsius: float,
    ...
    mapped = map_channel_params_to_braincell(
        channel_cls,
        params,
...
def normalize_config_schema(cfg: Dict[str, Any]) -> Dict[str, Any]:
    cfg = json.loads(json.dumps(cfg))
    gcfg = cfg.setdefault("global", {})
    ncfg = cfg.setdefault("neuron", {})
    bcfg = cfg.setdefault("braincell", {})

    legacy_geometry = cfg.pop("geometry", None)
    if isinstance(legacy_geometry, dict):
        for key in ("L_um", "diam_um", "cm_uF_cm2"):
            if key in legacy_geometry and key not in gcfg:
                gcfg[key] = legacy_geometry[key]

    if "ek_mV" in ncfg and "ion_rev_mV" not in gcfg:
        gcfg["ion_rev_mV"] = ncfg.pop("ek_mV")
    else:
        ncfg.pop("ek_mV", None)

    if "ion_E_mV" in bcfg and "ion_rev_mV" not in gcfg:
        gcfg["ion_rev_mV"] = bcfg.pop("ion_E_mV")
    else:
        bcfg.pop("ion_E_mV", None)

    if "pas" in ncfg and "leak" not in ncfg:
        ncfg["leak"] = ncfg.pop("pas")
    else:
        ncfg.pop("pas", None)
...
def load_config(config_path: Path) -> Dict[str, Any]:
    ...
    user_cfg = normalize_config_schema(user_cfg)
    cfg = normalize_config_schema(merge_dicts(DEFAULT_CONFIG, user_cfg))
...
def build_neuron_model(cfg: Dict[str, Any], case: Dict[str, float]) -> Dict[str, Any]:
    ...
    soma.L = float(gcfg["L_um"])
    soma.diam = float(gcfg["diam_um"])
    soma.nseg = 1
    soma.cm = float(gcfg["cm_uF_cm2"])

    leak = ncfg.get("leak")
    if leak is not None:
        soma.insert("pas")
        for seg in soma:
            seg.pas.g = float(leak["g_S_cm2"])
            seg.pas.e = float(leak["e_mV"])
    ...
    ion_field = resolve_neuron_ion_field(gcfg["ion_type"])
    setattr(soma, ion_field, float(gcfg["ion_rev_mV"]))
...
def run_case_neuron(cfg: Dict[str, Any], case: Dict[str, float]) -> Dict[str, np.ndarray]:
    ...
    # NEURON keeps the initial state sample when using Vector.record(...) + h.run().
    # Drop that first point so later comparisons align with braincell's step outputs.
    t_ms = t_ms[1:]
    v_mV = v_mV[1:]
    ik_outward = ik_outward[1:]
    gate_n = gate_n[1:]
...
def build_braincell_model(cfg: Dict[str, Any]):
    ...
    ncfg = cfg["neuron"]
    bcfg = cfg["braincell"]
    ion_type = str(gcfg["ion_type"]).lower()
    ion_attr = ion_type
    channel_slot = "channel"
    gate_var = ncfg.get("gate_var", "n")
    ion_cls = resolve_braincell_ion_class(ion_type)
    channel_cls = resolve_braincell_channel_class(bcfg["channel_name"])
    channel_kwargs = convert_braincell_channel_params(
        channel_cls,
        bcfg.get("channel_params", {}),
        temperature_celsius=float(gcfg["temperature_celsius"]),
    )
    leak = bcfg.get("leak")
    ...
    class KvCell(braincell.SingleCompartment):
        def __init__(self, size: int = 1):
            ...
            super().__init__(
                size,
                length=float(gcfg["L_um"]) * u.um,
                radius=0.5 * float(gcfg["diam_um"]) * u.um,
                C=float(gcfg["cm_uF_cm2"]) * u.uF / (u.cm ** 2),
                solver=bcfg["solver"],
                V_initializer=braintools.init.Uniform(v_init, v_init),
            )

            ion_group = ion_cls(size, E=float(gcfg["ion_rev_mV"]) * u.mV)
            ion_group.add(
                **{
                    channel_slot: channel_cls(
                        size,
                        **channel_kwargs,
                    )
                }
            )
            setattr(self, ion_attr, ion_group)
...
def run_case_braincell(cfg: Dict[str, Any], case: Dict[str, float]) -> Dict[str, np.ndarray]:
    ...
    model = build_braincell_model(cfg)
    cell = model["cell"]
    cell.init_state()
    cell.reset_state()
    ...
    def step_fun(t):
        ...
        i_nA = u.math.where(active, amp_nA, 0.0)
        with brainstate.environ.context(t=t):
            cell.update(i_nA * u.nA)
        v_now = cell.V.value
        ion_group = getattr(cell, model["ion_attr"])
        channel = getattr(ion_group, model["channel_slot"])
        ik_now = ion_group.current(v_now)
        gate_now = getattr(channel, model["gate_var"]).value
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Channel validation sweep helper for NEURON vs braincell.")
