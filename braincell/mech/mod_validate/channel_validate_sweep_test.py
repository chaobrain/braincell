...
def _load_template_module():
    script_path = Path(__file__).resolve().with_name("channel_validate_sweep.py")
    spec = importlib.util.spec_from_file_location("channel_validate_sweep", script_path)
...
def _load_module_by_name(filename: str):
...
def test_run_case_neuron_drops_initial_recorded_sample(monkeypatch):
    mod = _load_template_module()
    ...
    def fake_build_neuron_model(_cfg, _case):
        return {
            "h": FakeH(),
            "soma": object(),
            "t_vec": np.array([0.0, 0.025, 0.050], dtype=float),
            "v_vec": np.array([-65.0, -64.0, -63.0], dtype=float),
            "ik_outward_vec": np.array([0.0, -1.0, -2.0], dtype=float),
            "gate_vec": np.array([0.1, 0.2, 0.3], dtype=float),
        }

    monkeypatch.setattr(mod, "build_neuron_model", fake_build_neuron_model)

    trace = mod.run_case_neuron({}, {})

    assert np.allclose(trace["t_ms"], np.array([0.025, 0.050], dtype=float))
    assert np.allclose(trace["v_mV"], np.array([-64.0, -63.0], dtype=float))
    assert np.allclose(trace["ik_mA_cm2"], np.array([1.0, 2.0], dtype=float))
    assert np.allclose(trace["gate_n"], np.array([0.2, 0.3], dtype=float))


def test_load_config_normalizes_legacy_schema(tmp_path):
    mod = _load_template_module()
...
""".strip(),
    ...
    assert cfg["global"]["L_um"] == 12.0
    assert cfg["global"]["diam_um"] == 20.0
    assert cfg["global"]["cm_uF_cm2"] == 1.2
    assert cfg["global"]["ion_rev_mV"] == -77.0
    assert "geometry" not in cfg
    assert "ek_mV" not in cfg["neuron"]
    assert "ion_E_mV" not in cfg["braincell"]
    assert "pas" not in cfg["neuron"]
    assert cfg["neuron"]["leak"]["g_S_cm2"] == 0.0002
...
def test_new_entrypoint_modules_load():
    single = _load_module_by_name("channel_validate.py")
    sweep = _load_module_by_name("channel_validate_sweep.py")

    assert callable(single.main)
    assert callable(single.create_cell_neuron)
    assert callable(single.create_cell_braincell)
    assert callable(sweep.main)
    assert "global" in sweep.DEFAULT_CONFIG
    assert "geometry" not in sweep.DEFAULT_CONFIG
    assert "ion_type" in sweep.DEFAULT_CONFIG["global"]
    assert "ion_rev_mV" in sweep.DEFAULT_CONFIG["global"]
