...
- `pipeline/inspect_ir/variants/one_ion_hh_ohmic.py`
- `pipeline/render/variants/braincell_one_ion_hh_ohmic.py`
...
- multi-ion mechanisms such as `examples/hh.mod`
- GHK currents
...
1. extend Step 1 normalization in `pipeline/inspect_ast/`
2. add new Step 2 IR families in `pipeline/inspect_ir/`
3. add matching Step 3 render variants in `pipeline/render/`
4. keep each legal combination explicit in `pipeline/registry.py`
