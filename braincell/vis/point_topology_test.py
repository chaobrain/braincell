...
def _make_point_tree(*, lengths: tuple[float, float, float] = (20.0, 80.0, 60.0)):
    ...
    return build_point_tree(morpho, cvs=cvs)
...
class PointTopologyPlotTest(unittest.TestCase):
    ...
    def test_plot_point_topology_returns_axes(self) -> None:
        point_tree = _make_point_tree()
        ax = plot_point_topology(point_tree, layout="kamada_kawai")
        ...
        for bad_value in (0.0, -1.0, np.nan, np.inf):
            with self.subTest(bad_value=bad_value):
                with self.assertRaisesRegex(ValueError, "layout_scale must be a finite positive float"):
                    plot_point_topology(point_tree, layout="kamada_kawai", layout_scale=bad_value)
    ...
    def test_rejects_highlight_and_values_together(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "does not support highlight_point_ids together with values"):
            plot_point_topology(
                ...
                values=np.linspace(0.0, 1.0, len(point_tree.points)),
            )
    ...
    def test_values_shape_must_match_n_points(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "values must have shape"):
            plot_point_topology(
                ...
                layout="kamada_kawai",
                color_mode="values",
                values=np.ones(len(point_tree.points) - 1),
            )
    ...
    def test_graphviz_layout_falls_back_to_kamada_kawai(self) -> None:
        ...
        with mock.patch(
            ...
            with warnings.catch_warnings(record=True) as captured:
                ...
                ax = plot_point_topology(point_tree, layout="twopi")
        self.assertIsInstance(ax, matplotlib.axes.Axes)
    ...
    def test_same_topology_yields_same_layout_even_when_geometry_differs(self) -> None:
        point_tree_a = _make_point_tree(lengths=(20.0, 80.0, 60.0))
        point_tree_b = _make_point_tree(lengths=(50.0, 10.0, 200.0))

        ax_a = plot_point_topology(point_tree_a, layout="kamada_kawai")
        ax_b = plot_point_topology(point_tree_b, layout="kamada_kawai")
    ...
    def test_depth_color_mode_draws_all_points(self) -> None:
        point_tree = _make_point_tree()
        ax = plot_point_topology(point_tree, preset="depth", layout="kamada_kawai")
        offsets = ax.collections[-1].get_offsets()
        self.assertEqual(offsets.shape[0], len(point_tree.points))

    def test_kamada_kawai_layout_scale_changes_point_spacing(self) -> None:
        point_tree = _make_point_tree()
        ax_small = plot_point_topology(point_tree, layout="kamada_kawai", layout_scale=0.5)
        ax_large = plot_point_topology(point_tree, layout="kamada_kawai", layout_scale=2.0)
    ...
    def test_graphviz_layout_scale_scales_returned_positions(self) -> None:
        ...
        with mock.patch(
            ...
            ax = plot_point_topology(point_tree, layout="twopi", layout_scale=2.0)
    ...
    def test_graphviz_fallback_preserves_layout_scale(self) -> None:
        ...
        with mock.patch(
            ...
            with mock.patch(
                ...
                return_value={point.id: np.zeros(2, dtype=float) for point in point_tree.points},
            ) as mocked:
                plot_point_topology(point_tree, layout="twopi", layout_scale=2.5)
        mocked.assert_called_once()
        self.assertEqual(mocked.call_args.kwargs["layout_scale"], 2.5)

    def test_highlight_mode_draws_selected_overlay(self) -> None:
        point_tree = _make_point_tree()
        ax = plot_point_topology(point_tree, highlight_point_ids=[3], layout="kamada_kawai")
    ...
    def test_values_mode_can_draw_colorbar(self) -> None:
        ...
        ax = plot_point_topology(
            point_tree,
            values=np.linspace(-70.0, -50.0, len(point_tree.points)),
            value_label="V",
            show_colorbar=True,
        )
        self.assertIsInstance(ax, matplotlib.axes.Axes)
