...
本文是 BrainCell 参数学习的研究基线和候选设计，不是已经稳定的公共 API
规范。它回答两个近期问题：没有动作电位的电压数据怎样尽可能拟合好，以及数据
包含动作电位时怎样避免尖峰时间偏移主导训练。后续实验稳定后，可以再把参数
系统、损失函数、内存策略和在线学习拆成独立设计文档。
...
当前的
[heterogeneous nine-parameter training](../../../examples/experimental/parameter_learning/heterogeneous_nine_parameter_training.py)
...
其具体实验约束记录在
[Heterogeneous Nine-Parameter Training](../../specs/2026-08-17-heterogeneous-nine-parameter-training.md)。
...
假设 target spike 在 `t`，prediction spike 只晚了一个很小的 `delta t`。两条 spike
shape 可能几乎相同，但逐点相减会在一个位置得到 target peak 减 baseline，在另一
位置得到 baseline 减 predicted peak，产生两个很大的误差峰。参数稍微移动又会让
spike 数突然变化，因此 loss landscape 出现狭窄谷、台阶和不连续区域。
...
```text
L_spike = w_sub   * L_masked_subthreshold
        + w_align * L_alignment
        + w_event * L_filtered_events
        + w_shape * L_AP_features
        + w_reg   * L_regularization
```

各项定义：

- `L_masked_subthreshold`：由 target spike time 预先生成固定窗口，例如先测试
  `[-1, +3] ms`，窗口内降权、窗口外计算 Huber；
- `L_alignment`：对 sliding smooth-max/envelope 后的 voltage 使用 soft-DTW，或在
  短 trace 上直接使用 soft-DTW [3]；
- `L_filtered_events`：将 target 和 predicted event train 与指数或 Gaussian kernel
  卷积，再计算距离，kernel time constant 从 `1--3 ms` sweep。该思路对应
  van Rossum spike distance [4]；
- `L_AP_features`：spike count、first-spike latency、ISI、adaptation、amplitude、
  half-width、max `dV/dt` 和 AHP；
- `L_regularization`：同无 spike 路线。

soft-DTW 可微并允许时间轴上的局部对齐 [3]，但计算和存储成本随两条序列长度
近似二次增长。因此先做 window reduction 或限制 Sakoe-Chiba band 是必要的工程
选项。对齐成本应同时包含 voltage 与时间偏移惩罚，否则过强 warping 可能把错误的
spike pattern 对齐成低 loss。
...
| target-fixed mask | 保留 spike 外连续电压 | 永久使用会失去 active-channel 约束 | 早期或作为组合中的一项 |
...
1. 用 resting、steady-state、smooth peak 和 threshold margin 把模型带入正确兴奋
   性区域；
2. 加入 spike count、first latency 和 filtered event loss；
3. 加入 envelope soft-DTW，修正局部时间偏移；
4. spike 数稳定后加入 amplitude、half-width、`dV/dt` 和 AHP；
5. 最后在完整 trace 与全部 protocol 上微调，并按 held-out protocol 选模型。
...
3. 加入 spike timing offset、噪声、缺失/额外 spike 的受控扰动；
4. 再使用实验 recording，并用 feature-based 或 gradient-free result 做 baseline；
   BluePyOpt 是这类多目标生物物理参数优化的成熟参考实现 [5]；
5. 最后扩展到 density function 和在线学习。
...
- [Heterogeneous nine-parameter training](../../../examples/experimental/parameter_learning/heterogeneous_nine_parameter_training.py)
  及其
  [实验规格](../../specs/2026-08-17-heterogeneous-nine-parameter-training.md)。
- [Heterogeneous protocol dataset](../../../examples/experimental/parameter_learning/heterogeneous_protocol_dataset.py)
  及其
  [实验规格](../../specs/2026-08-17-heterogeneous-protocol-dataset.md)。
