# NetStim Contact Runtime

## Goal

Replace the implicit colocated `cell.place(..., NetStim(...))` route with an
explicit event-source/contact model that works for one cell and one-dimensional
cell populations. The same resolved contact representation must remain usable
by the network lowering layer.
...
- `NetStim` has no output weight. `Contact` owns target-compatible weight and
  delay.
...
- `Contact(source=..., target=..., weight=..., delay=..., pairs=None)` creates a
  Cell-owned declaration. It supports scalar fan-out/fan-in, equal-size zip,
  and explicit `(C, 2)` source/target row pairs.
...
- Contacts lower into columnar source indices, target runtime indices, weights,
  and delay steps. Multiple contacts may share a synapse instance and add their
  due payloads.
- Contact queues persist across continued `run()` calls and reset with the Cell.
- Structural edits remain declaration-time only.

## Compatibility

`cell.place(..., NetStim(...))` is removed. Existing deterministic NetStim
examples and tests migrate to standalone NetStim plus Contact. Existing
Network Projection APIs keep their public behavior while their delivery code
continues to share contact scatter/delay utilities.
...
- Contact pairing, unit validation, delay, shared target, continued-run, and
...
- A benchmark must report declaration/init/compile/warm-run time and persistent
  source/contact/synapse storage, then identify the dominant performance costs.

## Implemented result

- The CV-pair notebook is
  `examples/multi_compartment/netstim_heterogeneous_contacts.ipynb`; it executes
  a five-CV HH population with 10 CV-pair cells, 20 independently parameterized
  ExpSyn instances, and 20 heterogeneous NetStim contacts.
- `examples/multi_compartment/netstim_contact_benchmark.py` provides isolated
  ...
  measured source/contact/synapse columns. Warm three-step runs were about 400
