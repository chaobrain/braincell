"""CPU benchmark for broadcast and packed heterogeneous synapse layouts.
...
def synapse_declaration():
    return braincell.mech.SynapseSpec(
        "ExpSyn",
        tau=2.0 * u.ms,
        e=0.0 * u.mV,
        name="bench_exp",
...
def synapse_storage(cell) -> tuple[int, int, tuple[int, ...]]:
    ...
    shape = tuple(int(value) for value in cell.runtime.get_event_buffer(layout.id).shape)
...
def benchmark(mode: str, size: int, *, repeats: int, steps: int) -> dict[str, object]:
    ...
    return {
        ...
        "event_buffer_shape": state_shape,
