"""Scaling benchmark for heterogeneous synapse placement and NetStim contacts."""
...
def storage_summary(cell, sources, contact) -> dict[str, int]:
    """Return persistent source/contact/synapse storage in bytes."""
    ...
    return {
        ...
        "contact_bytes": int(contact_bytes),
        "synapse_bytes": int(synapse_bytes),
        "total_bytes": int(source_bytes + contact_bytes + synapse_bytes),
    }
...
def benchmark(size: int, *, heterogeneous: bool, steps: int = 5, repeats: int = 3):
    ...
    cell, sources, contact = build_model(size, heterogeneous=heterogeneous)
    declaration_s = time.perf_counter() - started

    started = time.perf_counter()
    cell.init_state()
    init_s = time.perf_counter() - started
    storage = storage_summary(cell, sources, contact)
