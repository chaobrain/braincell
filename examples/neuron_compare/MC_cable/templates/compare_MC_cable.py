#!/usr/bin/env python3
"""Compare NEURON vs braincell multi-compartment cable voltages."""
...
try:
    ...
    from braincell_single_case import run_case as run_braincell_case  # type: ignore
    from mapping import build_mapping  # type: ignore
    from neuron_single_case import run_case as run_neuron_case  # type: ignore
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run a multi-compartment cable voltage comparison case.",
    )
...
def _ensure_compatible(*, braincell_result: dict[str, Any], neuron_result: dict[str, Any]) -> None:
    ...
    if braincell_voltage.shape != neuron_voltage.shape:
        raise ValueError(
            f"braincell and NEURON voltage shapes do not match: "
