...
def build_mapping(
    ...
    *,
    ...
    morpho = _load_braincell_morphology(case)
    secs = _load_neuron_sections(case)
    try:
        ...
        _delete_neuron_sections(secs)


def _load_braincell_morphology(case):
    from braincell import Morphology

    return Morphology.from_swc(case.swc.path, mode="neuron")


def _load_neuron_sections(case):
    helper = _load_swc_morphology_helper()
    return helper(case.swc.path)


def _load_swc_morphology_helper():
    module_path = Path(__file__).resolve().parents[3] / "multi_compartment" / "neuron_diff.py"
    spec = importlib.util.spec_from_file_location("multi_compartment_neuron_diff_mapping", module_path)
...
def _delete_neuron_sections(secs) -> None:
...
def _braincell_descriptors(morpho) -> dict[int, TreeNodeDescriptor]:
    ...
    def build(branch_index: int) -> TreeNodeDescriptor:
        ...
        descriptor = TreeNodeDescriptor(
            ...
            child_type_multiset=tuple(sorted(child.branch_type for child in child_descriptors)),
            subtree_signature=(),
        )
