...
class WorkflowInputTest(unittest.TestCase):
    def test_repo_workflow_notebook_defaults_to_config_only(self) -> None:
        ...
        self.assertIn('config_path = CABLE_ROOT / "configs" / "IO.json"', parameter_cell)
        self.assertNotIn('template_path =', parameter_cell)
        self.assertIn('tables = workflow_api.build_summary_tables(selected_template_out_dir)', run_cell)
