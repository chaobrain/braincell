...
def _build_tree() -> Morpho:
    ...
    dend = Branch.lengths_shared(lengths=[100.0], radii=[2.0, 1.0], type="basal_dendrite")
...
class CellFacadeTest(unittest.TestCase):
    ...
    def test_cell_freezes_morphology_snapshot(self) -> None:
        ...
        cell = Cell(tree)
        self.assertEqual(cell.n_cv, 2)
        tree.soma.axon = Branch.lengths_shared(lengths=[80.0], radii=[1.0, 0.6], type="axon")
        self.assertEqual(cell.n_cv, 2)
    ...
    def test_cv_as_branch(self) -> None:
        ...
        self.assertGreater(float(branch_slice.total_length.to_decimal(u.um)), 0.0)
