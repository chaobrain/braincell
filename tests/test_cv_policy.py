...
def _build_three_branch_tree() -> Morpho:
    soma = Branch.lengths_shared(lengths=[20.0], radii=[8.0, 8.0], type="soma")
    dend_a = Branch.lengths_shared(lengths=[30.0], radii=[2.0, 1.5], type="basal_dendrite")
    dend_b = Branch.lengths_shared(lengths=[40.0], radii=[2.5, 1.0], type="apical_dendrite")
...
def _build_two_branch_tree() -> Morpho:
    soma = Branch.lengths_shared(lengths=[100.0], radii=[10.0, 8.0], type="soma")
    dend = Branch.lengths_shared(lengths=[45.0], radii=[2.0, 1.0], type="basal_dendrite")
    tree = Morpho.from_root(soma, name="soma")
    tree.soma.d = dend
    return tree
...
class CVPolicyTest(unittest.TestCase):
    ...
    def test_max_cv_len_preserves_cross_branch_topology(self) -> None:
        soma = Branch.lengths_shared(lengths=[10.0], radii=[3.0, 2.0], type="soma")
        dend = Branch.lengths_shared(lengths=[10.0], radii=[2.0, 1.0], type="basal_dendrite")
        tree = Morpho.from_root(soma, name="soma")
        tree.soma.d = dend
