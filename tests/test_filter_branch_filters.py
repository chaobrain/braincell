...
class BranchFilterTest(unittest.TestCase):
    def test_branch_in_filter_supports_type_and_name(self) -> None:
        ...
        region_name = BranchInFilter(property="name", values={"soma", "apical_dendrite_0"}).evaluate(tree)
    ...
    def test_branch_in_filter_supports_topology_properties(self) -> None:
        ...
        by_children = BranchInFilter(property="n_child", values=0).evaluate(tree)
    ...
    def test_branch_range_filter_supports_quantity_bounds(self) -> None:
        ...
        small_distal_radius = BranchRangeFilter(
            property="radius_distal",
            bounds=(None, 1.0 * u.um),
            closed="right",
        ).evaluate(tree)

        self.assertEqual(shorter.intervals, ((0, 0.0, 1.0), (1, 0.0, 1.0), (3, 0.0, 1.0)))
        self.assertEqual(small_distal_radius.intervals, ((1, 0.0, 1.0), (2, 0.0, 1.0), (3, 0.0, 1.0)))
    ...
    def test_invalid_conditions_raise_clear_errors(self) -> None:
        ...
        with self.assertRaises(ValueError):
            BranchRangeFilter(property="total_length", bounds=(0, 100), closed="both").evaluate(tree)
        with self.assertRaises(ValueError):
            BranchRangeFilter(property="n_child", bounds=(0 * u.um, 1 * u.um), closed="both").evaluate(tree)
        with self.assertRaises(ValueError):
