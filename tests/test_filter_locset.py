...
def _build_branchpoint_tree() -> Morpho:
    ...
    basal = Branch.lengths_shared(lengths=[30.0], radii=[2.0, 1.0], type="basal_dendrite")
    axon = Branch.lengths_shared(lengths=[10.0], radii=[0.8, 0.5], type="axon")
    apical = Branch.lengths_shared(lengths=[20.0], radii=[1.5, 1.0], type="apical_dendrite")
    tuft = Branch.lengths_shared(lengths=[15.0], radii=[0.9, 0.7], type="apical_dendrite")

    tree = Morpho.from_root(soma, name="soma")
    tree.soma.attach("basal", basal, parent_x=0.25)
    tree.soma.attach("axon", axon, parent_x=0.5)
    tree.soma.attach("apical", apical, parent_x=0.5)
    tree.soma.basal.tuft = tuft
    return tree


def _build_sampling_tree() -> Morpho:
    soma = Branch.lengths_shared(lengths=[10.0], radii=[10.0, 10.0], type="soma")
    dend = Branch.lengths_shared(lengths=[30.0], radii=[2.0, 1.0], type="basal_dendrite")
